...
static int soutnum_low_hex(char* s, const s32 n, const s32 base, struct params_s *par)
{
    int numLen = 0;
    s32 negative;
	s32 i;
        ...
        switch ((s32)ch) {
            ...
            case 'x': // soutnum_low_hex
                ...
                if (long_flag != 0) {
				    ...
				    sCounter += soutnum_low_hex(s+sCounter, (s32)va_arg(argp, s32), 16L, &par);
                }
                #else
                sCounter += soutnum_low_hex(s+sCounter, (s32)va_arg(argp, s32), 16L, &par);
                #endif
