...
reg r_clken_div; 
parameter CLKEN_DIV_PERIOD_SET = 8'd25; // 
reg [7:0] r_cnt_clk_div;
always @(posedge clk, negedge reset_n)
	if (!reset_n) begin
        r_clken_div   <= 1'b0;
...
always @(posedge clk, negedge reset_n)
	if (!reset_n) begin
		...
		// 
		STATE_SIG_0 : if (r_clken_div) begin
		...
		// 
		STATE_SIG_6 : if (r_clken_div) begin
			state				<= STATE_SIG_1;
			//
			r_rd_D              <=  8'b0; // [7:0]
			//
		...
		// 
		STATE_SIG_1 : if (r_clken_div) begin
			state				<= STATE_SIG_2;
		...
		// 
		STATE_SIG_2 : if (r_clken_div) begin
			// common
			r_fbit_index        <= r_fbit_index + 1;
			// shift data 
		...
		// 
		STATE_SIG_3 : if (r_clken_div) begin
			state				<= STATE_SIG_5;
			r_fbit_index        <= r_fbit_index + 1;
			r_CLK_SCLK          <=  1'b1;
			//
		...
		// 
		STATE_SIG_4 : if (r_clken_div) begin
			state				<= STATE_SIG_5;
			r_fbit_index        <= r_fbit_index + 1;
			r_CLK_SCLK          <=  1'b1;
			//
		...
		//
		STATE_SIG_5 : if (r_clken_div) begin
			state				<= STATE_SIG_0;
			r_fbit_index        <= 8'b0;
			//
			r_done_SPI_frame	<= 1'b1;
