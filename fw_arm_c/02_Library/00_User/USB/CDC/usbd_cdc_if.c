...
extern	void USB_CDC_RxCallback(uint8_t *pData, uint32_t *pLength);
...
extern	USBD_HandleTypeDef	hUSB_Device_FS;
...
static int8_t CDC_If_Init(void)
{
	USBD_CDC_SetRxBuffer(&hUSB_Device_FS, usbPacketData);
	USBD_CDC_SetTxBuffer(&hUSB_Device_FS, usbPacketData, 0);
	return (USBD_OK);
}
