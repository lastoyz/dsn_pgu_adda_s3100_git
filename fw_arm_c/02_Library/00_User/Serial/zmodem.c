...
#define TRACE		Ext_Serial_Printf
...
//#define Rxtimeout 10
//#define Rxtimeout (HAL_MAX_DELAY  - 1)
...
static int READLINE_PF(uint32_t timeout) 
{
	uint8_t data;
	int8_t res = 0;
...
static inline void zputhex(int c, u8 *pos)
{
...
//	if ( SerialIsReadyChar() ) ch = SerialIsGetChar();
//	ch  = READLINE_PF(0);
	if (HAL_UART_Receive(&h_Board_Serial, &ch, 1, 1) == HAL_OK)
		return ch;
	return -1;
}
...
static void zmputs(const char *s)
{
	...
	while (s && *s) {
		...
		if (!p) 
		{
			Serial_TxData( (u8*) s, strlen(s) );
			return;
		}
		if (p != s) 
		{
			Serial_TxData( (u8*)s, (size_t)(p-s) );
			s=p;
		}
		p++;
	}
...
static void canit(int fd)
{
	...
	purgeline(fd);
	Serial_TxData( (u8*)canistr, strlen(canistr) );
}
