...
ReturnType Driver_Init(FLASH_DEVICE_OBJECT *flash_device_object)
{
	...
	/* N25Q128 */
	if ((Device == MEM_TYPE_N25Q128) || (Device == MEM_TYPE_MX25U128))
	{
#ifdef DEBUG
	...
	/* N25Q256 */
	if ((Device == MEM_TYPE_N25Q256_V3) || (Device == MEM_TYPE_N25Q256_V18))
	{
#ifdef DEBUG
...
ReturnType DataRead(InstructionType insInstruction, ParameterType *fp)
{
	...
	rRetVal = FlashDataRead(fp->Read.udAddr, (u8*)fp->Read.pArray, fp->Read.udNrOfElementsToRead, insCode);
	return rRetVal;
}
...
*******************************************************************************/
ReturnType FlashDataProgram(uAddrType udAddr, NMX_uint8 *pArray , NMX_uint32 udNrOfElementsInArray, NMX_uint8 ubSpiInstruction)
{
	ReturnType retValue = Flash_Success;
	NMX_uint32 dataOffset;
...
*/
		...
		if((rTime - sTime) > sec)	return Flash_OperationTimeOut;
	}
...
//	NMX_uint8 sentBuffer[fdo->Desc.FlashOTPSize+1];
	volatile NMX_uint8 sentBuffer[0x80];
//	NMX_uint8 *sentBuffer;
	NMX_uint32 udAddr;
	ReturnType ret;

//	sentBuffer = (NMX_uint8*)SDRAM_DUMMY_3_ADRS;
